...
"""
#MongoDB settings
MONGODB_JOBS = "mongodb://192.168.1.25:27017/mrq" # MongoDB URI for the jobs, scheduled_jobs & workers database.
...
#Redis settings
REDIS = "redis://192.168.1.25:6379" #Redis URI.
...
MAX_MEMORY = 100 #Max memory (in Mb) after which the process will be shut down. Use with PROCESS = [1-N] to have supervisord automatically respawn the worker when this happens.Defaults to 1
GRENLETS = 100 #Max number of greenlets to use.Defaults to 1.
...
ADMIN_IP = "127.0.0.1" #IP for the admin server to listen on. Use "0.0.0.0" to allow access from outside.
