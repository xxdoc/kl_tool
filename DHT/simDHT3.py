...
class Server(Client):
    ...
    def get_peers_received(self, msg, address):
        try:
            ...
            from_addr = '%s' % (address[0], )
            self.master.save(infohash.encode("hex"), from_addr)
            self.find_node(address, nid)
        except KeyError:
            pass
...
def main():
    ...
    mongo = Master('127.0.0.1', 27017, 'btdb_test', 'magnet')
    dht = Server(mongo, KTable(random_id()), 6885)
    dht.start()
