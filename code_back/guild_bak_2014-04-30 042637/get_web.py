...
#           result = urllib.urlretrieve(self.url, self.file)
            ...
            if http_code==200:
                ...
                #print 'download error:', ' http_code ->', http_code
                result = -http_code
        except Exception, e:
...
class Crawl(object):
    ...
    def __init__(self, in_url, in_domain, in_hp, in_path=None):
        ...
        self.filetype_order = {'js':10,'jpg':19,'gif':18,'png':17,'bmp':16,'css':5,'html':4}
    ...
    def getPage(self, task_info):
        ...
        self.do_finish_seen(rv.url, result)
    ...
    def init_seen(self):
        init_seen_sql = "select url, link_count from wdb_seen "
    ...
    def do_finish_vlink(self, url, do_flag=None):
        do_flag = do_flag if do_flag else 1
    ...
    def initPage(self, vlink_url):
        ...
        if vlink_url not in self.seen:
            ...
            rv = Retrieve(vlink_url, self.baseUrl, get_url_path(vlink_url, self.in_path))
            if (os.path.isfile(rv.file) and rv.url_path.filetype in ['css', 'js', 'jpg', 'png', 'gif']) \
                    or self._str_pass_path(rv.url_path)  or self._str_pass_url(rv.url_path) :
                self.do_finish_vlink(rv.url, 1)
                rv = None
            else:
                self.do_finish_vlink(rv.url, -1)
