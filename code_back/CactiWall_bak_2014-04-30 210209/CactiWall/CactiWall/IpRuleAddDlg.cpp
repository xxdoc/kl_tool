...
BOOL CIpRuleAddDlg::OnInitDialog()
{
	...
	GetDlgItem( IDC_STATIC_T_IP)->ShowWindow( SW_HIDE );
	GetDlgItem( IDC_STATIC_R_IP)->ShowWindow( SW_HIDE );

	if(m_Reserved == M_RESERVED_REWRITE){
		GetDlgItem( IDC_RADIO_LOCAL_ANYIP)->ShowWindow( SW_HIDE );
		GetDlgItem( IDC_RADIO_REMOTE_ANYIP)->ShowWindow( SW_HIDE );
		GetDlgItem( IDC_RADIO_REMOTE_RANGEIP)->ShowWindow( SW_HIDE );

		GetDlgItem( IDC_STATIC_T_IP)->ShowWindow( SW_SHOW );
		GetDlgItem( IDC_STATIC_R_IP)->ShowWindow( SW_SHOW );
	}
...
BEGIN_MESSAGE_MAP(CIpRuleAddDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_RADIO_REWRITE, &CIpRuleAddDlg::OnBnClickedRadioRewrite)
	ON_BN_CLICKED(IDC_RADIO_DENY, &CIpRuleAddDlg::OnBnClickedRadioDeny)
END_MESSAGE_MAP()
...
void CIpRuleAddDlg::OnBnClickedRadioAllow()
{
	...
	GetDlgItem( IDC_RADIO_LOCAL_ANYIP)->ShowWindow( SW_SHOW );
	GetDlgItem( IDC_RADIO_REMOTE_ANYIP)->ShowWindow( SW_SHOW );
	GetDlgItem( IDC_RADIO_REMOTE_RANGEIP)->ShowWindow( SW_SHOW );

	GetDlgItem( IDC_STATIC_T_IP)->ShowWindow( SW_HIDE );
	GetDlgItem( IDC_STATIC_R_IP)->ShowWindow( SW_HIDE );
}

void CIpRuleAddDlg::OnBnClickedRadioDeny()
{
    // TODO: 在此添加控件通知处理程序代码
    m_bAllow = FALSE;
	m_Reserved = 0;
...
void CIpRuleAddDlg::OnBnClickedRadioRewrite()
{
	// TODO: 在此添加控件通知处理程序代码
	m_bAllow = TRUE;
	m_Reserved = M_RESERVED_REWRITE;

	((CButton *)GetDlgItem( IDC_RADIO_LOCAL_ANYIP))->SetCheck( FALSE );
	((CButton *)GetDlgItem( IDC_RADIO_LOCAL_UNIQUEIP))->SetCheck( TRUE );
	((CButton *)GetDlgItem( IDC_RADIO_LOCAL_RANGEIP))->SetCheck( FALSE );

    ((CButton *)GetDlgItem( IDC_RADIO_REMOTE_ANYIP))->SetCheck( FALSE );
	((CButton *)GetDlgItem( IDC_RADIO_REMOTE_UNIQUEIP))->SetCheck( TRUE );
	((CButton *)GetDlgItem( IDC_RADIO_REMOTE_RANGEIP))->SetCheck( FALSE );
