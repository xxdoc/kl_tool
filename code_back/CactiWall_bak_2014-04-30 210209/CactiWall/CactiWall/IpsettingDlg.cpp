...
void CIpSettingDlg::UpdateRuleList()
{
       ...
       i++)
    {
        ...
        {
			if( rule_elem.rule.Bits.Reserved == M_RESERVED_REWRITE )
			{
				m_RuleList.SetItemText( i,1,_T("重定向"));
			}
			else
			{
...
void CIpSettingDlg::OnBnClickedButtonAddrule()
{
    ...
    if( IDOK == dlg.DoModal())
    {
		...
		rule.rule.Bits.Reserved = dlg.m_Reserved;
...
void CIpSettingDlg::OnBnClickedButtonEditrule()
{
	...
	else if( m_RuleList.GetItemText( m_RuleList.GetSelectionMark(),1 )==_T("重定向"))
	{
        dlg.m_bAllow = TRUE;
		dlg.m_Reserved = M_RESERVED_REWRITE;
	}
	else
	{
		dlg.m_bAllow = FALSE;
