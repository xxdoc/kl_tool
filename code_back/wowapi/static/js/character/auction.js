...
var Auction = {
		...
		$.ajax({
            ...
            success: function(data) {
				if (data.error) {
					...
					if (data.item.currentBid >= data.item.buyout && data.item.buyout > 0) {
                        Auction.toast('won');
			...
			success: function(data) {
				if (data.error) {
					...
					if (Auction.page == 'auctions') {
						...
						Auction.increment('total-auctions-ended', 1, 50);
					}
				...
				if (data.claimedMail) {
					...
					$('#auction-'+ id).fadeOut('fast', function() {
						...
						if (Auction.page == 'bids') {
							...
							Auction.decrement('total-auctions-sold');
						}
					});
				}
			};
	 ...
	 * @param max
	 */
	increment: function(id, amount, max) {
		var source = $('#'+ id),
			data = source.html();
