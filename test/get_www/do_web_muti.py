    ...
    with Crawl(c, c.flag.flag_getonly, None, log_func_in=LOG) as cr:
        while cr.todo:
            ...
            try:
                ...
                continue
            tid += 1
            ret_html.append((html_str, charset, url, deep,))
            if len(ret_html)>=1:
                LOG('\n%s    Tid:%s(%s) >> Url:%s\n' % (T()[1], tid, run_count, url))
                #results_list = mutil(_do_ret, ret_html)
                results_list = (_do_ret[args_list[0]],)
                ret_html = []
                for tmp in results_list:
                    ex_code, url, deep, ret, msg = tmp
                    if ex_code<0:
