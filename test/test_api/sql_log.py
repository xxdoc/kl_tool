...
def _run(sql_file, save_seq, interval, group_path_func):
    ...
    with open(sql_file, 'r') as rf:
        for idx, line in enumerate(rf):
            ...
            sql_cmd, time_local, db_name, user, client_ip, cost_time, ret_num, thread_id = \
                tmp_list[0][1:-1], tmp_list[1], tmp_list[2], tmp_list[3], tmp_list[4], int(tmp_list[5]), int(tmp_list[6]), int(tmp_list[7].strip())
...
def main():
    print '\n---------------Start-------------------\n'
    sql_file = sys.argv[1].strip() if len(sys.argv) >= 2 and sys.argv[1].strip() else os.path.join(os.getcwd(), 'sql.log')
