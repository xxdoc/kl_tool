...
BASE_CAITIAO_LIST = [
    '[/assets/v2/img/upload/xianhua-xg.gif?v=jqsBXj2B]',
    '[/assets/v2/img/upload/geili-xg.gif?v=jqsBXj2B]',
    '[/assets/v2/img/upload/dingqi-xg.gif?v=9a1SxeZu]',
    '[/assets/v2/img/upload/dianzan-xg.gif?v=9a1SxeZu]',
    '[/assets/v2/img/upload/guzhang-xg.gif?v=9a1SxeZu]',
]

def send_msg(dms, room_id, api_key, uid, name):
    global BASE_CAITIAO_LIST, _id
    topic = 'r_test_chat_%d' % (room_id, )

    my_msg = random.choice(BASE_CAITIAO_LIST)

    if my_msg:
        # _LOG( u'%d...%s <%d>{%d} (%d)' % (uid, tmp_msg, BASE_MSG_DICT[tmp_msg], len(tmp_list), len(BASE_MSG_DICT)) )
        time_str = time.strftime("%H:%M", time.localtime())
        _id += 1
        ret = dms_pub(dms, room_id, uid, name, my_msg, 2)
        if ret:
            pass
